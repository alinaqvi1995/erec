    ...
    <section>
        <div class="container">
            <div class="row justify-content-center align-items-center py-lg-5 py-3">
                <div class="col">
                    <h1 class="mb-0 fs-48 text-center fw-bold text-uppercase mt-5 pt-5"> Browse Jobs </h1>
                </div>
            </div>
            <div class="row justify-content-center">
                <div class="col-lg-4">
                    <form action="{{ route('job.browse') }}" class="row row-cols-1 search-area mb-3 py-4 px-3 position-relative" id="search_job">
                        <div class="col">
                            <input type="text" name="search" class="w-100 fs-14 bg-theme-secondary px-5 text-dark h-50px mb-3" {{-- onchange="form.submit()" --}} placeholder="Search">
                            <img src="{{ asset('images/icon-search.svg') }}" alt="icon-search" class="img-fluid position-absolute">
                        </div>
                        {{-- <input type="hidden" name="search" value=""> --}}
                        <div class="col">
                            <input type="text" name="search_location" class="w-100 fs-14 bg-theme-secondary px-5 text-dark h-50px" {{-- onchange="form.submit()" --}} placeholder="All Locations">
                            <img src="{{ asset('images/icon-location.svg') }}" alt="icon-location" class="img-fluid position-absolute">
                        </div>
                    </form>

                    <div class="search-area mb-3 py-4 px-3 mb-3">
                ...
                <div class="col-lg-8">
                    <div class="d-lg-flex justify-content-between align-items-center py-2 mb-3 px-2">
                        <h3 class="mb-0 fs-5 fw-bold">Showing 1-{{ $app->total() }} Of {{ count($app)  }} Jobs</h3>
                                ...
                                <div class="card-body row align-items-center">
                                    <div class="col-lg-2">
                                        ...
                                        @if ($row->company->logo != NULL)
                                            <img src="{{ asset('storage/'.$row->company->logo) }}" alt="profile-img"  width="100" height="100" style="border-radius: 100px;">
                                        @else
                                    ...
                                    <div class="col-lg-7">
                                        <div class="details">
                                            <a href="{{ route('job.listing.details', $row->id) }}"><span class="title fw-bold text-theme-primary">{{ $row->post }}</span></a>
                                            <ul>
                                        ...
                                        <div class="btns d-flex flex-column">
                                            @if (auth()->check())
                                                @if (auth()->user()->role == "user")
                                                    <a class="btn open-apply-modal default-btn mb-3" id="{{ $row->id }}" href="javascript:;">Apply Now</a>
                                                @endif
                                            @else
                                                <a class="btn default-btn mb-3" href="{{ route('login') }}">Apply Now</a>
                                            @endif
                                            {{-- <a class="btn default-btn-2" href="{{ asset('candidateDoc/doc/'.$row->resume->document) }}" target="_blank">View Resume</a> --}}
                                            <div class="d-flex justify-content-between align-items-center">
                                                {{-- <a href="#"><img src="{{ asset('images/share.svg ') }}" alt="" class="img-fluid"></a> --}}
                                                <a href="mailto:{{ $row->company->user->email }}"><img src="{{ asset('images/mail.svg ') }}" alt="" class="img-fluid"></a>
                                            ...
                                            <div class="social-btn-sp d-flex justify-content-between align-items-center">
                                                {!! Share::page(route('job.listing.details', $row->id))->facebook()->twitter()->whatsapp()->linkedin() !!}
                                            </div>
                                        </div>
                                    </div>
                                </div>
